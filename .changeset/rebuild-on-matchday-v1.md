---
'@thewfa/api-client': major
---

Rebuilt the client against the new WFA Matchday API (public) OpenAPI spec.
...
- Removed the OAuth2 client (`MatchDayOAuthClient`) and Bearer/`accessToken` auth. The client now authenticates with an API key only.
